...
export default class AliyunHelper {
	...
	async describeInstances(filters: any): Promise<DescribeInstancesResponse> {
		let describeInstancesRequest = new $Ecs20140526.DescribeInstancesRequest({
			regionId: 'cn-shenzhen',
			instanceNetworkType: 'vpc',
		});
		let runtime = new $Util.RuntimeOptions(filters);
		// 复制代码运行请自行打印 API 的返回值
		return await this.client.describeInstancesWithOptions(describeInstancesRequest, runtime);
	}

	async listSecurityGroupRules(
		securityGroupId: string,
	): Promise<DescribeSecurityGroupAttributeResponse> {
		let describeSecurityGroupAttributeRequest =
			new $Ecs20140526.DescribeSecurityGroupAttributeRequest({
				regionId: this.region,
				securityGroupId: securityGroupId,
			});
		let runtime = new $Util.RuntimeOptions({});
		return await this.client.describeSecurityGroupAttributeWithOptions(
	...
	async updateSecurityGroupRule(
		...
		rule: any,
	): Promise<ModifySecurityGroupPolicyResponse> {
		...
		let runtime = new $Util.RuntimeOptions({});
		return await this.client.modifySecurityGroupRuleWithOptions(
